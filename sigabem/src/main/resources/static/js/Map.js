...
var app = new Vue({
		...
		todos:function(valor){
    		...
    		valor.map(stop => {
    			...
    			if(stop){
					...
					Lparadas.push(L.marker([stop.loc.lat, stop.loc.lon], {icon: this.LayersIcons.BusStopIcon}).on('click', ev =>{
						_self.estimativas=[{nome: "Carregando parada...", chegada: ""}]

						axios.get(`http://200.238.105.143:85/public/recife/stop/${stop.text}/estimations`).then(res =>{
	...
	methods:{
		buscarPontos(){
			...
			axios.get(`http://200.238.105.143:85/public/recife/stops?lat=${this.location.lat}&lon=${this.location.lon}&meters=${this.searchRadius}`).then(response => {
		...
		getLinhas(resStop){
			...
			axios.all(resStop.data.map(function(result) {
				return axios.get(`http://200.238.105.143:85/public/recife/stop/${result}/lines`).then(function (response) {
		...
		getStopCords(resStopLine, self){
			...
			axios.all(resStopLine.map(function(result) {
				return axios.get(`http://200.238.105.143:85/public/recife/line/${result.line}`).then(function (response) {
		...
		redirecionar(){
			...
			axios.get(`https://nominatim.openstreetmap.org/?addressdetails=1&q=${this.pesquisa}&format=json&limit=1`).then(res =>{
				let locali = {
					lat: res.data[0].lat,
					lon: res.data[0].lon
				}
	...
	mounted(){
		...
		navigator.geolocation.getCurrentPosition(position => {
			...
			L.tileLayer('http://c.tile.openstreetmap.org/{z}/{x}/{y}.png').addTo(this.map);
			...
			this.arcLayerMount(12, "Auditiva", this.LayersIcons.AuditivaIcon, "#9C2BCB")
	        this.arcLayerMount(13, "Fisica", this.LayersIcons.FisicaIcon, "#2AAD27")
	        this.arcLayerMount(14, "Intelectual", this.LayersIcons.IntelectualIcon, "#2A81CB")
	        this.arcLayerMount(15, "Multipla", this.LayersIcons.MultiplaIcon, "#CB8427")
