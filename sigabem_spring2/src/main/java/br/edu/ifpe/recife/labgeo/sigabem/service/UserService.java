...
public class UserService {
	...
	public List<User> findUsers(String deficiency) {
		
		RestTemplate restTemplate = new RestTemplate();
		Gson gson = new Gson();
		List<User> listaUsuario = null;
		
		try {
			URI uri = new URI("http://sigabem-env-1.f2p2pcstzh.us-east-2.elasticbeanstalk.com/users/login");
			
			HttpEntity<String> request = new HttpEntity<String>(gson.toJson(deficiency), new HttpHeaders());
			
			ResponseEntity<List<User>> usuarioResponse = restTemplate.exchange(uri, HttpMethod.GET, request, new ParameterizedTypeReference<List<User>>() {});
	       listaUsuario = usuarioResponse.getBody();
